...
count = conn.execute("SELECT COUNT(*) FROM spy_daily_prices").fetchone()[0]
...
total_days = len(rows)
winning_days = sum(1 for date, open_price, high_price in rows if high_price >= open_price + TARGET_GAIN)
losing_days = total_days - winning_days
success_rate = (winning_days / total_days * 100) if total_days > 0 else 0
...
print(f"Profit Per Win: ${profit_per_win}")
